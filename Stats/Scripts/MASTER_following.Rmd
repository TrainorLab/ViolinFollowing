...
```{r violin, fig.height=5.5, fig.width=8}
...
```{r allplots, fig.height=8, fig.width=7}
...
# Full (2) vs. reduced (1)
print(anova(mod.gc1[[1]], mod.gc1[[2]])) # p-value 
#anova(mod.gc1[[2]])
...
```{r}
anova(mod.cc2[[1]], mod.cc2[[2]]) # p-value
#anova(mod.cc2[[2]])
mod.cc2[[2]]
summary(mod.cc2[[2]])
...
```
```{r}
trials <- unique(following1$Trial)
...
cc.fit <- ggplot(following2, mapping = aes(x=Trial, y=CC)) +
  geom_point()
