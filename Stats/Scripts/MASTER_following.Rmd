...
imageDirectory <- "/Users/lucas/Desktop/Following/Manuscript/Images/" # save images to manuscript directory where LaTeX doc can access them
...
```{r}
# Are there any NAs?
# if colSums(is.na(following1)) ~= 0:
#   print('No NAs found')
# else:
#   print('There are NAs for piece 1')
# 
...
Recording to performance direction = 1 Performance to recording direction = 2

```{r}
source("Scripts/prep_following.R")
dfs <- prep_following(following1, following2) # list of 6 items: lf1, lf2, cc1, cc2, lf_comb, cc_comb
...
```{r}
# SEPARATING BY PLAYING EXPERIENCE - folk players vs. non-folk players
...
} else if(piece=="In The Garden" && section=="whole") {
...
```{r violin, fig.height=4.5, fig.width=6.5, fig.cap="Violin plots for both pieces showing differences in GC values across both directions."}
...
```{r GC_combined, fig.height=4.5, fig.width=6}
...
#plots[[6]] # CC values over trials, separated by piece
```

## Best lags by trial

```{r lags, fig.height=4, fig.width=6.5, fig.cap="Means of all participants' best time lags across trials for both pieces."}
#source("Scripts/plots_following.R")
#plots <- plots_following(dfs)
#plots[[14]] # Lags for both pieces on same graph

CC_combined_lags <- plots[[14]] + 
  facet_wrap(vars(Piece), labeller = as_labeller(plot_names), strip.position = "top") + 
  ggtitle("Piece") +
  scale_color_manual(values=c('darkorchid4','springgreen4'))
CC_combined_lags

ggsave(paste(imageDirectory,"bestlags.jpg",sep=''),CC_combined_lags)
```

```{r lags_sd, fig.height=4, fig.width=6.5}
lags1.sd <- data.frame(1:8,tapply(cc1$CC_l, cc1$Trial, sd))
names(lags1.sd) <- c("Trial","SD_bestlag")
lags1.sd$Piece <- 1

lags2.sd <- data.frame(1:8,tapply(cc2$CC_l, cc2$Trial, sd))
names(lags2.sd) <- c("Trial","SD_bestlag")
lags2.sd$Piece <- 2

lags.sd <- rbind(lags1.sd,lags2.sd)
lags.sd$Piece <- factor(lags.sd$Piece, ordered = TRUE)
...
lags_sd <- ggplot(lags.sd, aes(x=Trial, y=SD_bestlag, color=Piece)) +
  geom_line(size = 1) +
  geom_point() +
  labs(y = "SD of best time lag", color = "Piece") +
  ggtitle("St. Dev. of Maximum Time Lags") +
  scale_color_manual(values=c('orange','darkblue')) +
  #scale_color_manual(values = c("black", "gray"), labels=c("Trial 1", "Trial 8")) +
  thm
lags_sd

ggsave(paste(imageDirectory,"lags_sd.jpg",sep=''),lags_sd)
```
...
# TRIAL MUST BE AN INTEGER FOR LINEAR MODELS
lf1_r2p$Trial <- as.integer(lf1_r2p$Trial)
lf2_r2p$Trial <- as.integer(lf2_r2p$Trial)
cc1$Trial <- as.integer(cc1$Trial)
cc2$Trial <- as.integer(cc2$Trial)
...
#mod.gc1 <- lmer_following(following1) # Danny Boy
mod.gc2 <- lmer_following(lf2_r2p) # In The Garden
#mod.gc2 <- lmer_following(following2) # In The Garden
...
#### Linear models for Cross-correlation (full, not windowed)
...
#### Linear mixed models for best lags across trials:

```{r}
# ANOVAs between full (2) vs. reduced (1) models for best lags:
anova.cc1.L <- anova(mod.cc1[[11]], mod.cc1[[12]]) # p-value
anova.cc2.L <- anova(mod.cc2[[11]], mod.cc2[[12]]) # p-value
print(anova.cc1.L)
print(anova.cc2.L)

# Define variables to report in results
DBmcc_beta.L <- round(fixef(mod.cc1[[12]])[2], digits = 3)
DBmcc_rsq.L <- round(r2beta(mod.cc1[[12]], method = 'kr')[[7]][1], digits = 3) # or 'nsj'
DBmcc_pval.L <- round(anova.cc1.L$`Pr(>Chisq)`[2], digits = 3)

ITGmcc_beta.L <- round(fixef(mod.cc2[[12]])[2], digits = 3)
ITGmcc_rsq.L <- round(r2beta(mod.cc2[[12]], method = 'kr')[[7]][1], digits = 3) # or 'nsj'
ITGmcc_pval.L <- round(anova.cc2.L$`Pr(>Chisq)`[2], digits = 3)
```

```{r}
print(paste('Danny Boy: beta = ',DBmcc_beta.L,', R-squared = ',DBmcc_rsq.L,', p-value = ',DBmcc_pval.L))

print(paste('In The Garden: beta = ',ITGmcc_beta.L,', R-squared = ',ITGmcc_rsq.L,', p-value = ',ITGmcc_pval.L))
```
...
Calculating linear contrasts (comparisons) among levels of a quantitative (ordered) factor Assumes equal subjects in each group
...
DBaov_pval <- round(DBaov_summary[[1]][2,5], digits = 5)

ITGaov_df <- 1
ITGaov_dfres <- aov.gc2$df.residual
ITGaov_F <- round(ITGaov_summary[[1]][2,4], digits = 3)
ITGaov_pval <- round(ITGaov_summary[[1]][2,5], digits = 5)
...
#### Similarity - CC (full function, not windowed) using best lag
...
DBaovcc_pval <- round(DBaovcc_summary[[1]][2,5], digits = 5)

ITGaovcc_df <- 1
ITGaovcc_dfres <- aov.cc2$df.residual
ITGaovcc_F <- round(ITGaovcc_summary[[1]][2,4], digits = 3)
ITGaovcc_pval <- round(ITGaovcc_summary[[1]][2,5], digits = 5)
...
#### Synchrony - CC (full function, not windowed) using 0 lag
...
DBaovcc0_pval <- round(DBaovcc0_summary[[1]][2,5], digits = 5)

ITGaovcc0_df <- 1
ITGaovcc0_dfres <- aov.cc02$df.residual
ITGaovcc0_F <- round(ITGaovcc0_summary[[1]][2,4], digits = 3)
ITGaovcc0_pval <- round(ITGaovcc0_summary[[1]][2,5], digits = 5)
```
...
#### Trend analysis of best lags across trials
...
DBaovcc0L_pval <- round(DBaovcc0L_summary[[1]][2,5], digits = 5)

ITGaovcc0L_df <- 1
ITGaovcc0L_dfres <- aov.cc02L$df.residual
ITGaovcc0L_F <- round(ITGaovcc0L_summary[[1]][2,4], digits = 3)
ITGaovcc0L_pval <- round(ITGaovcc0L_summary[[1]][2,5], digits = 5)
```
...
Save entire workspace image (all variables) as .Rdata file Save data frames as .rda files

```{r}
## SAVE DATA
save.image("/Users/lucas/Desktop/Following/ANALYSIS/Stats/following.Rdata")
save(dfs, file = "following_dfs.rda")
save(plots, file = "following_plots.rda")
