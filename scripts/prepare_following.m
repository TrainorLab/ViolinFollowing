function [P_prep, S_prep] = prepare_following(P,S)
...
% 
% ds_factor = round(fs/ds_target); % 5513
...
%% TRUNCATE
trunc = min(length(P),length(S));
P_prep = P(1:trunc,1);
S_prep = S(1:trunc,1);

% Optional plotting to check downsampling and z-score

if plotting_flag == 1
    data_ds_v = data_ds(:,1)
    sr = 1/mean(diff(timePoints_ds));
    plot(timePoints_ds,data_z,'or')
    hold off
end
